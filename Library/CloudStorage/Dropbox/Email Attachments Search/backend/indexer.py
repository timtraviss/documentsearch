...
def scan_pdfs(folder, progress_callback=None):
    ...
    the signature: progress_callback(absolute_path, index, total_estimate_or_none)
    """
    docs = []
    # Collect file list first so callers can know total count if desired
    pdf_files = []
    ...
    for idx, (root, filename) in enumerate(pdf_files, start=1):
        ...
        if progress_callback:
            try:
                progress_callback(absolute_path, idx, total)
            except Exception:
